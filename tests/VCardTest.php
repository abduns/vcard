...
it('supports ADR with type', function () {
    $out = VCard::make()->addAddress('', 'Suite 1', '123 Main St', 'New York', 'NY', '10001', 'USA', 'WORK')->build();
...
it('supports TEL with type', function () {
    $out = VCard::make()->addPhoneNumber('+1234567890', 'CELL')->build();
...
it('supports TZ', function () {
    $out = VCard::make()->addTz('America/New_York')->build();
    expect($out)->toContain('TZ:America/New_York');
});

it('supports GEO', function () {
    $out = VCard::make()->addGeo('geo:37.386013,-122.082932')->build();
...
it('supports KEY via URL', function () {
    $out = VCard::make()->addKey('https://example.com/key.asc', 'application/pgp-keys')->build();
...
it('supports FBURL', function () {
    $out = VCard::make()->addFburl('https://cal.example.com/busy/jdoe', 'WORK')->build();
    expect($out)->toContain('FBURL;TYPE=WORK:https://cal.example.com/busy/jdoe');
});

it('supports CALADRURI', function () {
    $out = VCard::make()->addCaladruri('mailto:jdoe@example.com', 'WORK')->build();
    expect($out)->toContain('CALADRURI;TYPE=WORK:mailto:jdoe@example.com');
});

it('supports CALURI', function () {
    $out = VCard::make()->addCaluri('https://cal.example.com/jdoe', 'WORK')->build();
...
it('supports custom X- properties', function () {
    $out = VCard::make()->addProperty('X-CUSTOM', 'myvalue')->build();
